...
export const commandHandler = async ({ me, note, command }: params) => {
  ...
  const romajiPrompt = await kuroshiro.convert(command.prompt, { to: "romaji", mode: "normal", romajiSystem: "passport" })
  if (note.user.id === me.id) return console.log("Wait. Its me.")
  match(romajiPrompt)
    .with("konnichiha", () => me.reply(note.id, `こんにちは，${note.user.username}さん！`))
}
